...
module.exports = async (client, interaction, args) => {
    ...
    client.embed({
        title: `👪・Adopción`,
        desc: `${author} tiene ${target} pidió que lo adoptaran! \n${target} haga clic en uno de los botones`,
        components: [row],
        content: `${target}`,
        type: 'editreply',
    }, interaction)
    ...
    interaction.channel.awaitMessageComponent({ filter, componentType: Discord.ComponentType.Button, time: 60000 }).then(async i => {
        if (i.customId == "adopt_yes") {
            ...
            client.embed({
                title: `👪・Adopción - Aprobado`,
                desc: `${author} es ahora el orgulloso padre de ${target}! 🎉`,
                components: [],
                type: 'editreply'
            }, interaction);
        }
        ...
        client.embed({
            title: `👪・Adopción - Denegada`,
            desc: `${target} no ha contestado nada! Se cancela la adopción`,
            components: [],
            type: 'editreply'
        }, interaction);
    });
