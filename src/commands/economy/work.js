...
module.exports = async (client, interaction, args) => {
  ...
  Schema2.findOne({ Guild: interaction.guild.id, User: user.id }, async (err, dataTime) => {
    if (dataTime && dataTime.Work !== null && timeout - (Date.now() - dataTime.Work) > 0) {
      ...
      client.succNormal({ text: `Usted trabajó como ${replies[result]} y ganó: **${client.emotes.economy.coins} $${amount}**`, type: 'editreply' }, interaction);

      client.succNormal({
        text: `Has trabajado y ganado dinero!`,
        fields: [
          {
            name: `🦹‍♂️┆Crimen`,
