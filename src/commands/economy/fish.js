...
module.exports = async (client, interaction, args) => {
    ...
    if (!userItems || userItems.FishingRod == false) return client.errNormal({ error: "Tienes que comprar una caña de pescar!", type: 'editreply' }, interaction);
    ...
    if (userItems) {
        if (userItems.FishingRodUsage >= randrod) {
            ...
            return client.errNormal({ error: "Se te ha roto la caña de pescar. Ve a comprar una nueva!", type: 'editreply' }, interaction);
        }
    }
    ...
    Schema2.findOne({ Guild: interaction.guild.id, User: user.id }, async (err, dataTime) => {
        if (dataTime && dataTime.Fish !== null && timeout - (Date.now() - dataTime.Fish) > 0) {
            ...
            client.succNormal({ text: `Has pescado y has conseguido un ${fishToWin}`, type: 'editreply' }, interaction);
