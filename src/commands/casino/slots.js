...
module.exports = async (client, interaction, args) => {
    ...
    Schema.findOne({ Guild: interaction.guild.id, User: user.id }, async (err, data) => {
        if (data) {
            let money = parseInt(interaction.options.getNumber('cantidad'));
            let win = false;

            if (!money) return client.errUsage({ usage: "slots [cantidad]", type: 'editreply' }, interaction);
            ...
            if (win) {
                ...
                client.embed({
                    title: `🎰・Slots`,
                    desc: `Has Ganado **${client.emotes.economy.coins} $${money}**`,
                ...
                client.embed({
                    title: `🎰・Slots`,
                    desc: `Has Perdido **${client.emotes.economy.coins} $${money}**`,
                    components: [row], 
