...
module.exports = async (client, interaction, args) => {
    ...
    Schema.findOne({ Text: text, User: interaction.user.id, endTime: endtime }, async (err, data) => {
        if (data) {
            return client.errNormal({ error: `Ya has hecho este recordatorio!`, type: 'editreply' }, interaction);
        }
        else {
            return client.succNormal({
                text: `Su recordatorio está fijado!`,
                fields: [{
                    name: `${client.emotes.normal.clock}┇Fin de los tiempos`,
