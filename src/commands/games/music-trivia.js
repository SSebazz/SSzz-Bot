...
module.exports = async (client, interaction, args) => {
    ...
    if (!channel) return client.errNormal({ error: `No estás en un canal de voz!`, type: 'editreply' }, interaction);
    ...
    if (interaction.client.playerManager.get(interaction.guild.id)) return client.errNormal({ error: `No se puede utilizar mientras se reproduce una pista!`, type: 'editreply' }, interaction);

    if (interaction.client.triviaManager.get(interaction.guildId)) return client.errNormal({ error: `Ya hay un trivial en juego!`, type: 'editreply' }, interaction);
...
async function handleSubscription(interaction, player, client) {
    ...
    client.embed({
        title: `🎶・Music Quiz`,
        desc: `Prepárate. Vota para saltarte la canción introduciendo la palabra 'skip'. Mucha suerte. \n\n**Canciones en cola:** ${queue.length} \n**Thora de jugar:** 30 seconds`,
        type: 'editreply'
    }, interaction);
}
...
function getRandom(arr, n) {
    var result = new Array(n),
        ...
        taken = new Array(len);
    if (n > len)
        throw new RangeError('getRandom: Más elementos tomados que disponibles!');
    while (n--) {
