...
process.on('unhandledRejection', error => {
    ...
    const embed = new Discord.EmbedBuilder()
        .setTitle(`🚨・Rechazo de promesa no gestionada`)
        .addFields([
            {
                name: "Error",
...
process.on('warning', warn => {
    ...
    warnLogs.send({
        ...
        console.log('Error al enviar aviso a webhook')
        console.log(warn)
    })
});
...
client.on(Discord.ShardEvents.Error, error => {
    console.log(error)
    if (error) if (error.length > 950) error = error.slice(0, 950) + '... ver consola para más detalles';
    if (error.stack) if (error.stack.length > 950) error.stack = error.stack.slice(0, 950) + '... ver consola para más detalles';
    if (!error.stack) return
    const embed = new Discord.EmbedBuilder()
        .setTitle(`🚨・Una conexión websocket ha encontrado un error`)
        .addFields([
            {
                name: `Error`,
                value: `\`\`\`${error}\`\`\``,
